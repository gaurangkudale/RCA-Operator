...
echo "  kubectl logs -n rca-system deploy/rca-operator-controller-manager -c manager \\"
echo "    | grep -i 'CPUThrottling\\|ResourceSaturation'"
echo ""
info "Clean up:"
echo "  kubectl delete deploy $DEPLOYMENT_NAME -n $NAMESPACE"
echo "  kubectl delete incidentreports -n $NAMESPACE -l rca.rca-operator.tech/incident-type=ResourceSaturation"
