...
- **Automatic correlation rule detection** — `internal/autodetect` package mines the correlation buffer for recurring signal co-occurrence patterns and auto-creates `RCACorrelationRule` CRDs when confidence thresholds are met; includes pattern mining, accumulator, CRD lifecycle management, expiry, startup recovery, Prometheus metrics, and Helm integration (`--enable-autodetect`)
