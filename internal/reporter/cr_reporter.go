...
// EnsureIncidentWithRCA is like EnsureIncident but also sets root cause analysis
// fields (RootCause, DetectionMethod, Confidence) on the IncidentReport status.
// This is used by the anomaly detector to provide automatic root cause hypotheses.
func (r *Reporter) EnsureIncidentWithRCA(
	ctx context.Context,
	namespace, podName, agentRef, incidentType, severity, summary, dedupKey string,
	occurredAt time.Time,
	rootCause, detectionMethod, confidence string,
) error {
	active, err := r.findOpenIncident(ctx, namespace, podName, incidentType)
	if err != nil {
		return err
	...
	if active != nil {
		return r.updateActiveIncidentWithRCA(ctx, active, dedupKey, podName, severity, summary, rootCause, detectionMethod, confidence)
	}
	...
	if resolved != nil {
		return r.reopenIncidentWithRCA(ctx, resolved, dedupKey, podName, severity, summary, rootCause, detectionMethod, confidence)
	}
	...
	if err := r.client.Create(ctx, report); err != nil {
		return fmt.Errorf("failed to create IncidentReport with RCA: %w", err)
	}
	// Populate the Registry dedup cache for namespace-scoped incident types.
	if incidentType == IncidentTypeRegistry {
		r.openRegistryByNS[namespace] = report.Name
	}
	...
	if err := r.client.Status().Patch(ctx, report, client.MergeFrom(statusBase)); err != nil {
		return fmt.Errorf("failed to patch IncidentReport status with RCA: %w", err)
	}

	r.log.Info("Created IncidentReport with RCA",
		"namespace", namespace,
		"name", report.Name,
		"incidentType", incidentType,
		"severity", severity,
	...
	if r.Recorder != nil {
		r.Recorder.Eventf(report, corev1.EventTypeWarning, "IncidentDetected",
			"New %s incident detected via %s (confidence=%s) severity=%s: %s",
			incidentType, detectionMethod, confidence, severity, summary)
	}
	return nil
}

// updateActiveIncidentWithRCA updates an active incident with RCA fields.
func (r *Reporter) updateActiveIncidentWithRCA(
	ctx context.Context,
	report *rcav1alpha1.IncidentReport,
	dedupKey, podName, severity, summary, rootCause, detectionMethod, confidence string,
) error {
	now := r.Now()
	...
	if err := r.client.Patch(ctx, report, client.MergeFrom(metaBase)); err != nil {
		return fmt.Errorf("failed to patch IncidentReport metadata with RCA: %w", err)
	}
	...
	// Update RCA fields if provided and not already set
	if rootCause != "" && report.Status.RootCause == "" {
		report.Status.RootCause = rootCause
	}
	if detectionMethod != "" && report.Status.DetectionMethod == "" {
		report.Status.DetectionMethod = detectionMethod
	}
	if confidence != "" && report.Status.Confidence == "" {
		report.Status.Confidence = confidence
	}
	...
	r.log.Info("Updated active IncidentReport with RCA",
		"namespace", report.Namespace,
		"name", report.Name,
	)
	return nil
...
// reopenIncidentWithRCA reopens a resolved incident with RCA fields.
func (r *Reporter) reopenIncidentWithRCA(
	ctx context.Context,
	report *rcav1alpha1.IncidentReport,
	dedupKey, podName, severity, summary, rootCause, detectionMethod, confidence string,
) error {
	...
	report.Annotations[AnnotationDedupKey] = dedupKey
	if err := r.client.Patch(ctx, report, client.MergeFrom(metaBase)); err != nil {
		return fmt.Errorf("failed to patch IncidentReport metadata on reopen with RCA: %w", err)
	}
	...
	if err := r.client.Status().Patch(ctx, report, client.MergeFrom(statusBase)); err != nil {
		return fmt.Errorf("failed to patch IncidentReport status on reopen with RCA: %w", err)
	}

	r.log.Info("Reopened resolved IncidentReport with RCA",
		"namespace", report.Namespace,
		"name", report.Name,
	...
	if r.Recorder != nil {
		r.Recorder.Eventf(report, corev1.EventTypeWarning, "IncidentReopened",
			"Incident re-opened via %s: %s", detectionMethod, summary)
	}
