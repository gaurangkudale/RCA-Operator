...
func (r *Reporter) createIncident(ctx context.Context, input incident.Input, fingerprint, fingerprintHash string) error {
	...
	if r.Recorder != nil {
		r.Recorder.Publish(events.Event{
			...
			Message: fmt.Sprintf("New %s incident detected severity=%s: %s", input.IncidentType, input.Severity, input.Summary),
			Object:  report,
		})
	}
	return nil
...
func (r *Reporter) reopenIncident(ctx context.Context, report *rcav1alpha1.IncidentReport, input incident.Input, fingerprint, hash string) error {
	...
	if r.Recorder != nil {
		r.Recorder.Publish(events.Event{
			Type:    corev1.EventTypeWarning,
			Reason:  "IncidentReopened",
			Message: fmt.Sprintf("Incident re-opened: %s", input.Summary),
			Object:  report,
		})
	}
	return nil
